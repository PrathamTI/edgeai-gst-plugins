/*
 * GStreamer TI TVM Inference Plugin Implementation
 * Copyright (C) 2026 Texas Instruments Incorporated - http://www.ti.com/
 *
 * Replicates tvm_inference_client functionality as GStreamer element
 * Follows edgeai-gst-plugins patterns and conventions
 */
...
static void
gst_ti_tvm_get_property (GObject * object, guint property_id, GValue * value,
    GParamSpec * pspec);
static void
...
static
    GstFlowReturn
gst_ti_tvm_transform_ip (GstBaseTransform * trans, GstBuffer * buf);
...
/* Property enum defined in header file */
...
G_DEFINE_TYPE_WITH_CODE (GstTiTvm, gst_ti_tvm, GST_TYPE_BASE_TRANSFORM,
    GST_DEBUG_CATEGORY_INIT (gst_ti_tvm_debug_category,
        "titvm", 0, "TI TVM inference element"));
...
gst_ti_tvm_class_init (GstTiTvmClass * klass)
{
  ...
  base_transform_class->transform_ip =
      GST_DEBUG_FUNCPTR (gst_ti_tvm_transform_ip);

  /*
   * In non-passthrough (transform) mode GstBaseTransform calls
   * prepare_output_buf before transform().  That function checks
   * gst_pad_get_current_caps(srcpad); if caps aren't established yet
   * it returns GST_FLOW_NOT_NEGOTIATED before our code ever runs.
   * In-place mode (transform_ip) skips prepare_output_buf entirely:
   * the input buffer is made writable and passed directly to transform_ip.
   * This is correct because GCRN output size == input size. */
...
{
  gst_base_transform_set_in_place (GST_BASE_TRANSFORM (tvm), TRUE);
...
{
  /*
   * Simply nulling these pointers skips the destructors, so the TVM
   * ObjectRef refcount never reaches zero and the DSP never unloads the
   * model.  After a few failed/restarted runs the DSP runs out of memory
   * (DYN_LOAD_MEM status=-9) and all subsequent runs fail.
   * Calling delete invokes the destructor which decrements the refcount;
   * when it reaches zero TVM tells the C7x DSP to release the allocation. */
  GstTiTvm *
      tvm = GST_TI_TVM (trans);
...
gst_ti_tvm_transform_ip (GstBaseTransform * trans, GstBuffer * buf)
{
  ...
  if (!gst_buffer_map (buf, &map_info, GST_MAP_READWRITE)) {
    GST_ERROR_OBJECT (tvm, "Failed to map buffer");
    return GST_FLOW_ERROR;
  }

  gfloat *
      tensor_data = (gfloat *) map_info.data;
  gsize tensor_size = map_info.size / sizeof (gfloat);
  GST_DEBUG_OBJECT (tvm, "transform_ip: %zu floats (%zu bytes)", tensor_size,
      map_info.size);

  /* Run inference benchmark — runs multiple iterations, tracks timing */
  GstFlowReturn
      ret = gst_ti_tvm_run_inference_benchmark (tvm, tensor_data, tensor_size);

  /* Overwrite the buffer with the inference output */
  if (ret == GST_FLOW_OK && tvm->final_output && tvm->output_num_floats > 0) {
    ...
    out->CopyToBytes (tensor_data, out_bytes);
    gst_buffer_set_size (buf, (gssize) out_bytes);
  }
